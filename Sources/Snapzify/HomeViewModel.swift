    ...
    func pasteImage() {
        ...
        Task {
            ...
            do {
                ...
                print("Failed to process pasted image: \(error)")
            }
        }
    }
    ...
    func processPickedImage(_ image: UIImage) {
        Task {
            ...
            do {
                ...
                let document = try await processImage(image, source: .imported)
                print("Image processed, saving document with \(document.sentences.count) sentences...")
                try await store.save(document)
                print("Document saved successfully")
                await loadDocuments()
                print("Documents reloaded")
            } catch {
                print("Failed to process picked image: \(error)")
                await MainActor.run {
