...
struct HomeView: View {
    ...
    var body: some View {
        ...
        .onChange(of: appState.shouldRefreshDocuments) { shouldRefresh in
            if shouldRefresh {
                Task {
                    await vm.loadDocuments()
                    await MainActor.run {
                        appState.shouldRefreshDocuments = false
                    }
                }
            }
        }
        .alert("Clear All History", isPresented: $vm.showClearHistoryAlert) {
            ...
            Text("This will permanently delete all snapzified screenshots and translations. This action cannot be undone.")
        }
    }
