...
class TTSServiceOpenAI: TTSService {
    ...
    private var pendingRequests: [String: Task<AudioAsset, Error>] = [:]
    private let requestQueue = DispatchQueue(label: "tts.request.queue", attributes: .concurrent)
    ...
    func generateAudio(for text: String, script: ChineseScript) async throws -> AudioAsset {
        ...
        let speed = 1.0
        let cacheKey = "\(text):\(voice):\(script.rawValue):\(speed)".sha256()
        
        if let existingAsset = checkCache(for: cacheKey) {
            return existingAsset
        }
        
        return try await requestQueue.sync {
            if let existingTask = pendingRequests[cacheKey] {
                return try await existingTask.value
            }
            
            let task = Task<AudioAsset, Error> {
                let audioData = try await requestTTS(text: text, voice: voice, speed: speed)
                let asset = try saveAudio(data: audioData, key: cacheKey)
